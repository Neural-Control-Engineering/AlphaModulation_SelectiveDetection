function out = instBetaPhaseEachSpike(ap_data, lfp_data)
    out = ap_data;
    for t = 1:size(ap_data,1)
        ...
        for c = 1:size(ap_data(t,:).spiking_data{1},1)
            ...
            y = bandpassFilter(lfp, 13, 30, 500);
            phi = angle(hilbert(y));
        ...
        out(t,:).spiking_data{1} = [out(t,:).spiking_data{1}, table(phases, 'VariableNames', {'spike_beta_phase'})];
    end
end