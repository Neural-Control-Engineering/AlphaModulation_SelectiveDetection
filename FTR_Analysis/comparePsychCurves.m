function comparePsychCurves(ftr_files, visualize, out_path, ftr_names)
    % compare animals 
    ...
    if length(ftr_files) == 1
        expr = sprintf('mat%i = cell2mat(ftr%i.response_probability);', i, i);
        eval(expr)
    ...
    if out_path 
        saveas(fig, 'psych_curve_comparison.fig')
    end
