function evokedAlphaModulation(ap_session, visualize, out_path)

    if out_path
        fig_path = strcat(out_path, 'Evoked_Alpha_Modulation/');
        mkdir(fig_path)
    ...
    p_value = nan(size(ap_session,1),1);
    p_value_hit = nan(size(ap_session,1),1);
    p_value_miss = nan(size(ap_session,1),1);
    p_value_cr = nan(size(ap_session,1),1);
    p_value_fa = nan(size(ap_session,1),1);

    for c = 1:size(ap_session,1)
        % spontaneous
        if ~isempty(ap_session(c,:).evoked_alpha_spike_phases{1})
            [p, ~] = circ_rtest(ap_session(c,:).evoked_alpha_spike_phases{1});
            p_value(c) = p;
        else
            p_value(c) = nan;
        end
        ...
        % hit trials 
        if ~isempty(ap_session(c,:).evoked_alpha_spike_phases_hit{1})
            [p, ~] = circ_rtest(ap_session(c,:).evoked_alpha_spike_phases_hit{1});
            p_value_hit(c) = p;
        else
            p_value_hit(c) = nan;
        end
        % miss trials 
        if ~isempty(ap_session(c,:).evoked_alpha_spike_phases_miss{1})
            [p, ~] = circ_rtest(ap_session(c,:).evoked_alpha_spike_phases_miss{1});
            p_value_miss(c) = p;
        else
            p_value_miss(c) = nan;
        end
        % CR trials
        if ~isempty(ap_session(c,:).evoked_alpha_spike_phases_cr{1}) 
            [p, ~] = circ_rtest(ap_session(c,:).evoked_alpha_spike_phases_cr{1});
            p_value_cr(c) = p;
        else
            p_value_cr(c) = nan;
        end
        % FA trials 
        if ~isempty(ap_session(c,:).evoked_alpha_spike_phases_fa{1})
            [p, ~] = circ_rtest(ap_session(c,:).evoked_alpha_spike_phases_fa{1});
            p_value_fa(c) = p;
        else
            p_value_fa(c) = nan;
        end
    ...
    theta_bars = nan(size(alpha_modulated,1),1);
    Rs = nan(size(alpha_modulated,1),1);
    kappas = nan(size(alpha_modulated,1),1);
    theta_bars_hit = nan(size(alpha_modulated,1),1);
    Rs_hit = nan(size(alpha_modulated,1),1);
    kappas_hit = nan(size(alpha_modulated,1),1);
    theta_bars_miss = nan(size(alpha_modulated,1),1);
    Rs_miss = nan(size(alpha_modulated,1),1);
    kappas_miss = nan(size(alpha_modulated,1),1);
    theta_bars_cr = nan(size(alpha_modulated,1),1);
    Rs_cr = nan(size(alpha_modulated,1),1);
    kappas_cr = nan(size(alpha_modulated,1),1);
    theta_bars_fa = nan(size(alpha_modulated,1),1);
    Rs_fa = nan(size(alpha_modulated,1),1);
    kappas_fa = nan(size(alpha_modulated,1),1);
    pmi = nan(size(alpha_modulated,1),1);
    pmi_hit = nan(size(alpha_modulated,1),1);
    pmi_miss = nan(size(alpha_modulated,1),1);
    pmi_cr = nan(size(alpha_modulated,1),1);
    pmi_fa = nan(size(alpha_modulated,1),1);
    pmi_correct = nan(size(alpha_modulated,1),1);
    pmi_incorrect = nan(size(alpha_modulated,1),1);
    pmi_action = nan(size(alpha_modulated,1),1);
    pmi_inaction = nan(size(alpha_modulated,1),1);
    mses = nan(size(alpha_modulated,1),1);
    mses_hit = nan(size(alpha_modulated,1),1);
    mses_miss = nan(size(alpha_modulated,1),1);
    mses_cr = nan(size(alpha_modulated,1),1);
    mses_fa = nan(size(alpha_modulated,1),1);
    mses_correct = nan(size(alpha_modulated,1),1);
    mses_incorrect = nan(size(alpha_modulated,1),1);
    mses_action = nan(size(alpha_modulated,1),1);
    mses_inaction = nan(size(alpha_modulated,1),1);
    p_correct = nan(size(alpha_modulated,1),1);
    p_incorrect = nan(size(alpha_modulated,1),1);
    p_action = nan(size(alpha_modulated,1),1);
    p_inaction = nan(size(alpha_modulated,1),1);

    for i = 1:size(alpha_modulated,1)
        % overall spontaneous phase modulation 
        %% computations 
        [N, edges] = histcounts(alpha_modulated(i,:).evoked_alpha_spike_phases_hit{1}, 20, 'Normalization', 'pdf');
        centers = zeros(length(edges)-1,1);
        for e = 1:(length(edges)-1)
            centers(e) = mean(edges(e:(e+1)));
        end
        [x,y, theta_bars(i), Rs(i), kappas(i)] = vonMises(alpha_modulated(i,:).evoked_alpha_spike_phases{1});
        ...
        axs(1,1) = nexttile;
        plot(linspace(-2.95,4.95,80), alpha_modulated(i,:).left_trigger_aligned_avg_fr_Hit{1}, 'k')
        title('Hit')
        xlim([-3,5])
        axs(1,2) = nexttile;
        plot(linspace(-2.95,4.95,80), alpha_modulated(i,:).left_trigger_aligned_avg_fr_Miss{1}, 'k')
        title('Miss')
        xlim([-3,5])
        axs(1,3) = nexttile;
        plot(linspace(-2.95,4.95,80), alpha_modulated(i,:).right_trigger_aligned_avg_fr_CR{1}, 'k')
        title('CR')
        xlim([-3,5])
        axs(1,4) = nexttile;
        plot(linspace(-2.95,4.95,80), alpha_modulated(i,:).right_trigger_aligned_avg_fr_FA{1}, 'k')
        title('FA')
        xlim([-3,5])
        unifyYLimits(axs(1,:))
        axs(2,1) = nexttile;
        %%% plotting phase modulation
        if ~isempty(alpha_modulated(i,:).evoked_alpha_spike_phases_hit{1})
            [N, edges] = histcounts(alpha_modulated(i,:).evoked_alpha_spike_phases_hit{1}, 20, 'Normalization', 'pdf');
            centers = zeros(length(edges)-1,1);
            for e = 1:(length(edges)-1)
                centers(e) = mean(edges(e:(e+1)));
            end
            [x,y, theta_bars_hit(i), Rs_hit(i), kappas_hit(i)] = vonMises(alpha_modulated(i,:).evoked_alpha_spike_phases_hit{1});
        ...
        axs(2,2) = nexttile;
        if ~isempty(alpha_modulated(i,:).evoked_alpha_spike_phases_miss{1})   
            [N, edges] = histcounts(alpha_modulated(i,:).evoked_alpha_spike_phases_miss{1}, 20, 'Normalization', 'pdf');
            centers = zeros(length(edges)-1,1);
            for e = 1:(length(edges)-1)
                centers(e) = mean(edges(e:(e+1)));
            end
            [x,y, theta_bars_miss(i), Rs_miss(i), kappas_miss(i)] = vonMises(alpha_modulated(i,:).evoked_alpha_spike_phases_miss{1});
        ...
        axs(2,3) = nexttile;
        if ~isempty(alpha_modulated(i,:).evoked_alpha_spike_phases_cr{1})
            [N, edges] = histcounts(alpha_modulated(i,:).evoked_alpha_spike_phases_cr{1}, 20, 'Normalization', 'pdf');
            centers = zeros(length(edges)-1,1);
            for e = 1:(length(edges)-1)
                centers(e) = mean(edges(e:(e+1)));
            end
            [x,y, theta_bars_cr(i), Rs_cr(i), kappas_cr(i)] = vonMises(alpha_modulated(i,:).evoked_alpha_spike_phases_cr{1});
        ...
        axs(2,4) = nexttile;
        if ~isempty(alpha_modulated(i,:).evoked_alpha_spike_phases_fa{1})
            [N, edges] = histcounts(alpha_modulated(i,:).evoked_alpha_spike_phases_fa{1}, 20, 'Normalization', 'pdf');
            centers = zeros(length(edges)-1,1);
            for e = 1:(length(edges)-1)
                centers(e) = mean(edges(e:(e+1)));
            end
            [x,y, theta_bars_fa(i), Rs_fa(i), kappas_fa(i)] = vonMises(alpha_modulated(i,:).evoked_alpha_spike_phases_fa{1});
        ...
        correct = [alpha_modulated(i,:).evoked_alpha_spike_phases_hit{1}, alpha_modulated(i,:).evoked_alpha_spike_phases_cr{1}];
        incorrect = [alpha_modulated(i,:).evoked_alpha_spike_phases_miss{1}, alpha_modulated(i,:).evoked_alpha_spike_phases_fa{1}];
        ...
        action = [alpha_modulated(i,:).evoked_alpha_spike_phases_hit{1}, alpha_modulated(i,:).evoked_alpha_spike_phases_fa{1}];
        inaction = [alpha_modulated(i,:).evoked_alpha_spike_phases_miss{1}, alpha_modulated(i,:).evoked_alpha_spike_phases_cr{1}];
        ...
        if startsWith(alpha_modulated(i,:).region{1}, 'SS')
            t = sprintf('Cortex: %s', alpha_modulated(i,:).waveform_class{1});
