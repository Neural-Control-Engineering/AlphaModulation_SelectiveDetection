function spontaneousBetaModulation(ap_session, visualize, out_path)

    if out_path
        fig_path = strcat(out_path, 'Spontaneous_Beta_Modulation/');
        mkdir(fig_path)
    ...
    for c = 1:size(ap_session,1)
        % spontaneous
        if ~isempty(ap_session(c,:).spon_beta_spike_phases{1})
            [p, ~] = circ_rtest(ap_session(c,:).spon_beta_spike_phases{1});
            p_value(c) = p;
        else
            p_value(c) = nan;
        end
        ...
        % hit trials 
        if ~isempty(ap_session(c,:).spon_beta_spike_phases_hit{1})
            [p, ~] = circ_rtest(ap_session(c,:).spon_beta_spike_phases_hit{1});
            p_value_hit(c) = p;
        else
            p_value_hit(c) = nan;
        end
        % miss trials 
        if ~isempty(ap_session(c,:).spon_beta_spike_phases_miss{1})
            [p, ~] = circ_rtest(ap_session(c,:).spon_beta_spike_phases_miss{1});
            p_value_miss(c) = p;
        else
            p_value_miss(c) = nan;
        end
        % CR trials
        if ~isempty(ap_session(c,:).spon_beta_spike_phases_cr{1}) 
            [p, ~] = circ_rtest(ap_session(c,:).spon_beta_spike_phases_cr{1});
            p_value_cr(c) = p;
        else
            p_value_cr(c) = nan;
        end
        % FA trials 
        if ~isempty(ap_session(c,:).spon_beta_spike_phases_fa{1})
            [p, ~] = circ_rtest(ap_session(c,:).spon_beta_spike_phases_fa{1});
            p_value_fa(c) = p;
        else
            p_value_fa(c) = nan;
        end
    ...
    for i = 1:size(beta_modulated,1)
        ...
        %% computations 
        [N, edges] = histcounts(beta_modulated(i,:).spon_beta_spike_phases_hit{1}, 20, 'Normalization', 'pdf');
        centers = zeros(length(edges)-1,1);
        for e = 1:(length(edges)-1)
            centers(e) = mean(edges(e:(e+1)));
        end
        [x,y, theta_bars(i), Rs(i), kappas(i)] = vonMises(beta_modulated(i,:).spon_beta_spike_phases{1});
        ...
        if ~isempty(beta_modulated(i,:).spon_beta_spike_phases_hit{1})
            [N, edges] = histcounts(beta_modulated(i,:).spon_beta_spike_phases_hit{1}, 20, 'Normalization', 'pdf');
            centers = zeros(length(edges)-1,1);
            for e = 1:(length(edges)-1)
                centers(e) = mean(edges(e:(e+1)));
            end
            [x,y, theta_bars_hit(i), Rs_hit(i), kappas_hit(i)] = vonMises(beta_modulated(i,:).spon_beta_spike_phases_hit{1});
        ...
        axs(2,2) = nexttile;
        if ~isempty(beta_modulated(i,:).spon_beta_spike_phases_miss{1})   
            [N, edges] = histcounts(beta_modulated(i,:).spon_beta_spike_phases_miss{1}, 20, 'Normalization', 'pdf');
            centers = zeros(length(edges)-1,1);
            for e = 1:(length(edges)-1)
                centers(e) = mean(edges(e:(e+1)));
            end
            [x,y, theta_bars_miss(i), Rs_miss(i), kappas_miss(i)] = vonMises(beta_modulated(i,:).spon_beta_spike_phases_miss{1});
        ...
        axs(2,3) = nexttile;
        if ~isempty(beta_modulated(i,:).spon_beta_spike_phases_cr{1})
            [N, edges] = histcounts(beta_modulated(i,:).spon_beta_spike_phases_cr{1}, 20, 'Normalization', 'pdf');
            centers = zeros(length(edges)-1,1);
            for e = 1:(length(edges)-1)
                centers(e) = mean(edges(e:(e+1)));
            end
            [x,y, theta_bars_cr(i), Rs_cr(i), kappas_cr(i)] = vonMises(beta_modulated(i,:).spon_beta_spike_phases_cr{1});
        ...
        axs(2,4) = nexttile;
        if ~isempty(beta_modulated(i,:).spon_beta_spike_phases_fa{1})
            [N, edges] = histcounts(beta_modulated(i,:).spon_beta_spike_phases_fa{1}, 20, 'Normalization', 'pdf');
            centers = zeros(length(edges)-1,1);
            for e = 1:(length(edges)-1)
                centers(e) = mean(edges(e:(e+1)));
            end
            [x,y, theta_bars_fa(i), Rs_fa(i), kappas_fa(i)] = vonMises(beta_modulated(i,:).spon_beta_spike_phases_fa{1});
        ...
        correct = [beta_modulated(i,:).spon_beta_spike_phases_hit{1}, beta_modulated(i,:).spon_beta_spike_phases_cr{1}];
        incorrect = [beta_modulated(i,:).spon_beta_spike_phases_miss{1}, beta_modulated(i,:).spon_beta_spike_phases_fa{1}];
        ...
        action = [beta_modulated(i,:).spon_beta_spike_phases_hit{1}, beta_modulated(i,:).spon_beta_spike_phases_fa{1}];
        inaction = [beta_modulated(i,:).spon_beta_spike_phases_miss{1}, beta_modulated(i,:).spon_beta_spike_phases_cr{1}];
