function out = avgPSTHbyLeftMinusRight(npxls_session, slrt_data, ap_data, edges)
    out = npxls_session;
    ...
    for a = 1:length(lmr)
        amp = lmr(a);
        tmp_ap = ap_data(cell2mat(slrt_data.left_minus_right_amp) == amp, :);
        if ~isempty(tmp_ap)
            ...
            col_title = strcat('amp_', num2str(amp), '_avg_psth');
