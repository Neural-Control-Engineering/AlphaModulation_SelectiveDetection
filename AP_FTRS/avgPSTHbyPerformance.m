function out = avgPSTHbyPerformance(ap_session, slrt_data, ap_data, events, edges)
    % correct 
    ...
    if ~isempty(tmp_ap)
        for c = 1:size(ap_session, 1)
            ...
            avg_psths{c} = mean(psth_mat, 1);
        end
        out = [ap_session, table(avg_psths, 'VariableNames', {'stim_aligned_psth_correct'})];
    else
        out = ap_session;
    end
    ...
    if ~isempty(tmp_ap)
        ...
        out = [out, table(avg_psths, 'VariableNames', {'stim_aligned_psth_incorrect'})];
    end
