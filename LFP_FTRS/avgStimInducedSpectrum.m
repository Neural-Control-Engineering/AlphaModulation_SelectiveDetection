function out = avgStimInducedSpectrum(lfp_session, lfp_data, event_names)
    ...
    for e = 1:length(event_names)
        ...
        for c = 1:size(lfp_session,1)
            ...
            for t = 1:size(tmp_lfp,1)
                mat{t,1} = tmp_lfp(t,:).lfp{1}(c, tmp_lfp(t,:).(event_name){1} > 0 & tmp_lfp(t,:).(event_name){1} < 1.2);
            end
        ...
        time_title = strcat(col_title, '_induced_f');
