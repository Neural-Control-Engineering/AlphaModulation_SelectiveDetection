...
*** in AlphaModFigScripts/alphaModAndLicking.m 
last_session_id = '';
all_lick_frs = {};
all_no_lick_frs = {};
for nrn = 1:size(alpha_modulated,1)
    ...
    contains_lick = spontaneousLicks(slrt_data);
    lick_inds = find(contains_lick);
    lick_frs = nan(length(lick_inds),1);
    for li = 1:length(lick_inds)
        t = lick_inds(li);
    ...
    all_no_lick_frs{nrn} = no_lick_frs;
end
