function spontaneousCovarianceSummary(ap_data, slrt_data)
    outcomes = {'Hit', 'Miss', 'CR', 'FA'};
    actions = {{'Hit', 'FA'}, {'Miss', 'CR'}};
    performance = {{'Hit', 'CR'}, {'FA', 'Miss'}};

    avg_cov_mats = spontaneousCovarianceAnalysis(ap_data, slrt_data);

    [fig_outcome, axs_outcome, t_outcome, cb_outcome] = plotHeatmaps(avg_cov_mats(1:4), outcomes);
    adjustColorLimits(fig_outcome, [-5,5]);
    ...
    adjustColorLimits(fig_action, [-5,5]);
    ...
    adjustColorLimits(fig_performance, [-5,5]);
