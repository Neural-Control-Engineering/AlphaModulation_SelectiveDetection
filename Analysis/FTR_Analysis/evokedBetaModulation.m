function evokedBetaModulation(ap_session, visualize, out_path)

    if out_path
        fig_path = strcat(out_path, 'Evoked_Beta_Modulation/');
        mkdir(fig_path)
    ...
    for c = 1:size(ap_session,1)
        % evoked
        if ~isempty(ap_session(c,:).evoked_beta_spike_phases{1})
            [p, ~] = circ_rtest(ap_session(c,:).evoked_beta_spike_phases{1});
            p_value(c) = p;
        else
            p_value(c) = nan;
        end
        ...
        % hit trials 
        if ~isempty(ap_session(c,:).evoked_beta_spike_phases_hit{1})
            [p, ~] = circ_rtest(ap_session(c,:).evoked_beta_spike_phases_hit{1});
            p_value_hit(c) = p;
        else
            p_value_hit(c) = nan;
        end
        % miss trials 
        if ~isempty(ap_session(c,:).evoked_beta_spike_phases_miss{1})
            [p, ~] = circ_rtest(ap_session(c,:).evoked_beta_spike_phases_miss{1});
            p_value_miss(c) = p;
        else
            p_value_miss(c) = nan;
        end
        % CR trials
        if ~isempty(ap_session(c,:).evoked_beta_spike_phases_cr{1}) 
            [p, ~] = circ_rtest(ap_session(c,:).evoked_beta_spike_phases_cr{1});
            p_value_cr(c) = p;
        else
            p_value_cr(c) = nan;
        end
        % FA trials 
        if ~isempty(ap_session(c,:).evoked_beta_spike_phases_fa{1})
            [p, ~] = circ_rtest(ap_session(c,:).evoked_beta_spike_phases_fa{1});
            p_value_fa(c) = p;
        else
            p_value_fa(c) = nan;
        end
    ...
    % beta_modulated = ap_session(cell2mat(p_value) < cell2mat(p_threshold),:);
    beta_modulated = ap_session(p_value < (0.5 / size(ap_session,1)),:);
    pct_beta_modulated = size(beta_modulated,1) / size(ap_session,1);
    ...
    theta_bars = zeros(size(beta_modulated,1),1);
    Rs = zeros(size(beta_modulated,1),1);
    kappas = zeros(size(beta_modulated,1),1);
    theta_bars_hit = zeros(size(beta_modulated,1),1);
    Rs_hit = zeros(size(beta_modulated,1),1);
    kappas_hit = zeros(size(beta_modulated,1),1);
    theta_bars_miss = zeros(size(beta_modulated,1),1);
    Rs_miss = zeros(size(beta_modulated,1),1);
    kappas_miss = zeros(size(beta_modulated,1),1);
    theta_bars_cr = zeros(size(beta_modulated,1),1);
    Rs_cr = zeros(size(beta_modulated,1),1);
    kappas_cr = zeros(size(beta_modulated,1),1);
    theta_bars_fa = zeros(size(beta_modulated,1),1);
    Rs_fa = zeros(size(beta_modulated,1),1);
    kappas_fa = zeros(size(beta_modulated,1),1);
    pmi = zeros(size(beta_modulated,1),1);
    pmi_hit = zeros(size(beta_modulated,1),1);
    pmi_miss = zeros(size(beta_modulated,1),1);
    pmi_cr = zeros(size(beta_modulated,1),1);
    pmi_fa = zeros(size(beta_modulated,1),1);
    pmi_correct = zeros(size(beta_modulated,1),1);
    pmi_incorrect = zeros(size(beta_modulated,1),1);
    pmi_action = zeros(size(beta_modulated,1),1);
    pmi_inaction = zeros(size(beta_modulated,1),1);
    mses = zeros(size(beta_modulated,1),1);
    mses_hit = zeros(size(beta_modulated,1),1);
    mses_miss = zeros(size(beta_modulated,1),1);
    mses_cr = zeros(size(beta_modulated,1),1);
    mses_fa = zeros(size(beta_modulated,1),1);
    mses_correct = zeros(size(beta_modulated,1),1);
    mses_incorrect = zeros(size(beta_modulated,1),1);
    mses_action = zeros(size(beta_modulated,1),1);
    mses_inaction = zeros(size(beta_modulated,1),1);
    p_correct = zeros(size(beta_modulated,1),1);
    p_incorrect = zeros(size(beta_modulated,1),1);
    p_action = zeros(size(beta_modulated,1),1);
    p_inaction = zeros(size(beta_modulated,1),1);

    for i = 1:size(beta_modulated,1)
        % overall evoked phase modulation 
        %% computations 
        [N, edges] = histcounts(beta_modulated(i,:).evoked_beta_spike_phases_hit{1}, 20, 'Normalization', 'pdf');
        centers = zeros(length(edges)-1,1);
        for e = 1:(length(edges)-1)
            centers(e) = mean(edges(e:(e+1)));
        end
        [x,y, theta_bars(i), Rs(i), kappas(i)] = vonMises(beta_modulated(i,:).evoked_beta_spike_phases{1});
        ...
        xticks([-pi, 0, pi])
        xticklabels({'-\pi', '0', '\pi'})
        % title(sprintf('%s %s: p = %.2d; p-threshold=%.2d', beta_modulated(i,:).region{1}, beta_modulated(i,:).waveform_class{1}, p_value(i), p_threshold(i)))
        ...
        if out_path 
            fname = sprintf('%s_cluster_%i', beta_modulated(i,:).session_id{1}, beta_modulated(i,:).cluster_id);
        ...
        axs(1,1) = nexttile;
        plot(linspace(-2.95,4.95,80), beta_modulated(i,:).left_trigger_aligned_avg_fr_Hit{1}, 'k')
        title('Hit')
        xlim([-3,5])
        axs(1,2) = nexttile;
        plot(linspace(-2.95,4.95,80), beta_modulated(i,:).left_trigger_aligned_avg_fr_Miss{1}, 'k')
        title('Miss')
        xlim([-3,5])
        axs(1,3) = nexttile;
        plot(linspace(-2.95,4.95,80), beta_modulated(i,:).right_trigger_aligned_avg_fr_CR{1}, 'k')
        title('CR')
        xlim([-3,5])
        axs(1,4) = nexttile;
        plot(linspace(-2.95,4.95,80), beta_modulated(i,:).right_trigger_aligned_avg_fr_FA{1}, 'k')
        title('FA')
        xlim([-3,5])
        unifyYLimits(axs(1,:))
        axs(2,1) = nexttile;
        %%% plotting phase modulation
        if ~isempty(beta_modulated(i,:).evoked_beta_spike_phases_hit{1})
            [N, edges] = histcounts(beta_modulated(i,:).evoked_beta_spike_phases_hit{1}, 20, 'Normalization', 'pdf');
            centers = zeros(length(edges)-1,1);
            for e = 1:(length(edges)-1)
                centers(e) = mean(edges(e:(e+1)));
            end
            [x,y, theta_bars_hit(i), Rs_hit(i), kappas_hit(i)] = vonMises(beta_modulated(i,:).evoked_beta_spike_phases_hit{1});
        ...
        axs(2,2) = nexttile;
        if ~isempty(beta_modulated(i,:).evoked_beta_spike_phases_miss{1})   
            [N, edges] = histcounts(beta_modulated(i,:).evoked_beta_spike_phases_miss{1}, 20, 'Normalization', 'pdf');
            centers = zeros(length(edges)-1,1);
            for e = 1:(length(edges)-1)
                centers(e) = mean(edges(e:(e+1)));
            end
            [x,y, theta_bars_miss(i), Rs_miss(i), kappas_miss(i)] = vonMises(beta_modulated(i,:).evoked_beta_spike_phases_miss{1});
        ...
        axs(2,3) = nexttile;
        if ~isempty(beta_modulated(i,:).evoked_beta_spike_phases_cr{1})
            [N, edges] = histcounts(beta_modulated(i,:).evoked_beta_spike_phases_cr{1}, 20, 'Normalization', 'pdf');
            centers = zeros(length(edges)-1,1);
            for e = 1:(length(edges)-1)
                centers(e) = mean(edges(e:(e+1)));
            end
            [x,y, theta_bars_cr(i), Rs_cr(i), kappas_cr(i)] = vonMises(beta_modulated(i,:).evoked_beta_spike_phases_cr{1});
        ...
        axs(2,4) = nexttile;
        if ~isempty(beta_modulated(i,:).evoked_beta_spike_phases_fa{1})
            [N, edges] = histcounts(beta_modulated(i,:).evoked_beta_spike_phases_fa{1}, 20, 'Normalization', 'pdf');
            centers = zeros(length(edges)-1,1);
            for e = 1:(length(edges)-1)
                centers(e) = mean(edges(e:(e+1)));
            end
            [x,y, theta_bars_fa(i), Rs_fa(i), kappas_fa(i)] = vonMises(beta_modulated(i,:).evoked_beta_spike_phases_fa{1});
        ...
        if out_path
            fname = sprintf('%s_cluster_%i.fig', beta_modulated(i,:).session_id{1}, beta_modulated(i,:).cluster_id);
            saveas(outcome_fig, strcat(fig_path, 'Phase_Preference_By_Outcome/', fname))
            fname = sprintf('%s_cluster_%i.png', beta_modulated(i,:).session_id{1}, beta_modulated(i,:).cluster_id);
            saveas(outcome_fig, strcat(fig_path, 'Phase_Preference_By_Outcome/', fname))
        end
        ...
        correct = [beta_modulated(i,:).evoked_beta_spike_phases_hit{1}, beta_modulated(i,:).evoked_beta_spike_phases_cr{1}];
        incorrect = [beta_modulated(i,:).evoked_beta_spike_phases_miss{1}, beta_modulated(i,:).evoked_beta_spike_phases_fa{1}];
        ...
        if ~isempty(correct) && ~isempty(incorrect)
            ...
            title(correct_tl, sprintf('%s %s', beta_modulated(i,:).region{1}, beta_modulated(i,:).waveform_class{1}))
            if out_path
                fname = sprintf('%s_cluster_%i.fig', beta_modulated(i,:).session_id{1}, beta_modulated(i,:).cluster_id);
        ...
        action = [beta_modulated(i,:).evoked_beta_spike_phases_hit{1}, beta_modulated(i,:).evoked_beta_spike_phases_fa{1}];
        inaction = [beta_modulated(i,:).evoked_beta_spike_phases_miss{1}, beta_modulated(i,:).evoked_beta_spike_phases_cr{1}];
        ...
        if startsWith(beta_modulated(i,:).region{1}, 'SS')
            t = sprintf('Cortex: %s', beta_modulated(i,:).waveform_class{1});
        elseif strcmp(beta_modulated(i,:).region{1}, 'CP') || strcmp(beta_modulated(i,:).region{1}, 'STR')
            t = sprintf('Basal Ganglia: %s', beta_modulated(i,:).waveform_class{1});
        else
            t = sprintf('Amygdala: %s', beta_modulated(i,:).waveform_class{1});
        end
        title(tl_combo, t)
    ...
    bar([1,2], [sum(p_correct < overall_p_threshold) / size(beta_modulated,1), sum(p_incorrect < overall_p_threshold) / size(beta_modulated,1)], ...
    ...
    bar([1,2], [sum(p_action < overall_p_threshold) / size(beta_modulated,1), sum(p_inaction < overall_p_threshold) / size(beta_modulated,1)], ...
        'EdgeColor', [0.5,0.5,0.5], 'FaceColor', [0.5,0.5,0.5])
    ylabel('Fraction of Modulated Neurons')
    xticks([1,2])
    xticklabels({'Action', 'Inaction'})
    ...
    rs_frac = sum(strcmp(beta_modulated.waveform_class, 'RS')) / sum(strcmp(ap_session.waveform_class, 'RS'));
    fs_frac = sum(strcmp(beta_modulated.waveform_class, 'FS')) / sum(strcmp(ap_session.waveform_class, 'FS'));
    ps_frac = sum(strcmp(beta_modulated.waveform_class, 'PS')) / sum(strcmp(ap_session.waveform_class, 'PS'));
    ts_frac = sum(strcmp(beta_modulated.waveform_class, 'TS')) / sum(strcmp(ap_session.waveform_class, 'TS'));
    if visualize
    ...
    beta_modulated = [beta_modulated, ...
        table(theta_bars, ...
            ...
            'mses_action', ...
            'mses_inaction'})];
    beta_modulated = [beta_modulated, table(p_value, ...
                p_value_hit, ...
                p_value_miss, ...
                p_value_cr, ...
                p_value_fa, ...
    ...
    out = struct();
    out.beta_modulated = beta_modulated;
    out.rs_frac = rs_frac;
    out.fs_frac = fs_frac;
    out.ps_frac = ps_frac;
    out.ts_frac = ts_frac;
    out.pct_beta_modulated = pct_beta_modulated;
