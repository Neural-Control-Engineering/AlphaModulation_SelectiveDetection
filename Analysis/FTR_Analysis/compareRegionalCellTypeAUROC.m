function compareRegionalCellTypeAUROC(ap_session1, ap_session2, regions, ttl, visualize, out_path)
    ...
    for i = 1:length(cell_types)
        ...
        avgs1(i) = mean(cell2mat(ct1.auROC));
        errs1(i) = std(cell2mat(ct1.auROC)) / sqrt(size(ct2,1));
        avgs2(i) = mean(cell2mat(ct2.auROC));
        errs2(i) = std(cell2mat(ct2.auROC)) / sqrt(size(ct2,1));
    end
    ...
    title(ttl)
    if out_path
        fname = sprintf('%s_auroc_comparison', ttl);
