function compareCellTypeZscoreFRs(ap_session1, ap_session2, signals, labels, ttl, visualize, out_path)
    ...
    for r = 1:length(regions)
        ...
        for i = 1:length(cell_types)
            ...
            for j = 1:length(signals)
                ...
                axs(j) = nexttile;
                if ~isempty(fr_mat1)
                    mu = nanmean(fr_mat1(:,1:(zero_ind-1)),2);
                    sigma = nanstd(fr_mat1(:,1:(zero_ind-1)),0,2);
                    fr_mat1 = fr_mat1(sigma>0,:);
                    mu = mu(sigma>0);
                    sigma = sigma(sigma>0);
                    try
                        fr_mat1 = (fr_mat1-mu)./sigma;
                        try
                ...
                if ~isempty(fr_mat2)
                    mu = nanmean(fr_mat2(:,1:(zero_ind-1)),2);
                    sigma = nanstd(fr_mat2(:,1:(zero_ind-1)),0,2);
                    fr_mat2 = fr_mat2(sigma>0,:);
                    mu = mu(sigma>0);
                    sigma = sigma(sigma>0);
                    try
                        fr_mat2 = (fr_mat2-mu)./sigma;
                        try
            ...
            unifyYLimits(fig)
            if out_path
                fname = sprintf('%s_%s_%s_zscore_firing_rates_comparison', lower(ttl), lower(region), cell_type);
